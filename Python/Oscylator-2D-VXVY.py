...
plt.title("Oscylator harmoniczny 2D-VXVY, porównanie rozwiązań numerycznych", fontdict=font1)
plt.xlabel("VX", fontdict = font2)
plt.ylabel("VY", fontdict = font2)
...
def animation_frame(i):
    xdata.append(dokladneVX[i])
    ydata.append(dokladneVY[i])

    xdata2.append(eulerVX[i])
    ydata2.append(eulerVY[i])

    xdata3.append(posredniaVX[i])
    ydata3.append(posredniaVY[i])

    xdata4.append(verletVX[i])
    ydata4.append(verletVY[i])

    xdata5.append(beemanVX[i])
    ydata5.append(beemanVY[i])
